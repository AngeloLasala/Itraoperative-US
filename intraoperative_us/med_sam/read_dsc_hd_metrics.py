...
def main(args, experiment_list):
    ...
    experiment_dict = {exp: {"dsc": [], "hd": []} for exp in experiment_list}
    for s in range(5):
        print(f'Split {s}')
        ...
        for experiment in experiment_list:
            ...
            experiment_dict[experiment]["dsc"].extend(dsc_list.tolist())
            experiment_dict[experiment]["hd"].extend(hd_list.tolist())  
        

    ## STATISTICAL ANALYSIS OVER ALL SPLITS ################################################################
    dsc_exp = [experiment_dict[experiment]["dsc"] for experiment in experiment_list]
    hd_exp = [experiment_dict[experiment]["hd"] for experiment in experiment_list]
    stat_dsc, p_dsc = friedmanchisquare(*dsc_exp)
    stat_hd, p_hd = friedmanchisquare(*hd_exp)
    print()
    ...
    if p_dsc < 0.05:
        ...
        for i in range(len(experiment_list)):
            for j in range(i+1, len(experiment_list)):
                stat, p = wilcoxon(experiment_dict[experiment_list[i]]["dsc"], experiment_dict[experiment_list[j]]["dsc"])
    ...
    if p_hd < 0.05:
        ...
        for i in range(len(experiment_list)):
            for j in range(i+1, len(experiment_list)):
                stat, p = wilcoxon(experiment_dict[experiment_list[i]]["hd"], experiment_dict[experiment_list[j]]["hd"])
                if p < alpha_corrected:
                    print(f'    {experiment_list[i]} vs {experiment_list[j]}: stat={stat:.3f}, p={p:.3f}, Significant')
                else:
                    print(f'    {experiment_list[i]} vs {experiment_list[j]}: stat={stat:.3f}, p={p:.3f}, Not Significant')
    ...
    if p_dsc < 0.05:
        ...
        pairs_dsc_sig = [pair for pair, p in zip(pairs, pvals_dsc_corr) if p < alpha]
        pvals_dsc_sig = [p for p in pvals_dsc_corr if p < alpha]

        if pairs_dsc_sig:
            print(f'pairs sig: {pairs_dsc_sig} - pvals sig: {pvals_dsc_sig}')
            annotator = Annotator(axes[0], pairs_dsc_sig, data=df_dsc_melt,
                                x="Experiment", y="DSC")
            annotator.configure(text_format='star', loc='inside', verbose=0,
                                line_width=1.5,
                                fontsize=20) 
            annotator.set_pvalues(pvals_dsc_sig)
            annotator.annotate()
    ...
    if p_hd < 0.05:
        ...
        pairs_hd_sig = [pair for pair, p in zip(pairs, pvals_hd_corr) if p < alpha]
        pvals_hd_sig = [p for p in pvals_hd_corr if p < alpha]

        if pairs_hd_sig:
            print(f'pairs sig: {pairs_hd_sig} - pvals sig: {pvals_hd_sig}')
            annotator = Annotator(axes[1], pairs_hd_sig, data=df_hd_melt,
                                x="Experiment", y="HD")
            annotator.configure(text_format='star', loc='inside', verbose=0,
                                line_width=1.5,
                                fontsize=20) 
            annotator.set_pvalues(pvals_hd_sig)
            annotator.annotate()
    ...
    # ######################################################################################################
        ...
        # # # Per split statistical analysis   ################################################################################################
