...
def kid_experiment(dataset_path,  splitting_json, experiment_dir, batch_size=50, device=None, dims=2048, num_workers=4):
...
if __name__ == "__main__":
    ...
    kid_scores = kid_experiment(args.dataset_path, args.splitting_json, experiment_dir_w, batch_size=50, device=device)

    for ep, (mean, std) in kid_scores.items():
        print(f"Epoch {ep}: KID = {mean:.6f} ± {std:.6f}")

    ## save the KID score
    with open(os.path.join(experiment_dir, f'w_{args.guide_w}', f'KID_score_{args.scheduler}.txt'), 'w') as f:
        for ep, (mean, std) in kid_scores.items():
