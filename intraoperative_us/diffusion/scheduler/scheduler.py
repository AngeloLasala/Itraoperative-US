...
if __name__ == '__main__':
    ...
    scheduler = LinearNoiseScheduler(1000, 0.0001, 0.001)
    image = mask.to(device) 
    noise = torch.randn_like(image)
    # t = torch.tensor([0 for _ in range(8)])
    # add_noise = scheduler.add_noise(image, noise, t)
    ...
    ddim.save_pretrained("ddim")
    # print(ddim)

    ddim = DDIMScheduler.from_pretrained("ddim")
    fig, ax = plt.subplots(1, 4, figsize=(16, 4), num='DDIM')
    for ii, t in enumerate([0,10,100,999]):
        t = torch.tensor([ii for _ in range(8)])
        add_noise = ddim.add_noise(image, noise, t)
        ax[ii].imshow(add_noise[0, 0].cpu().numpy())


    ## PNDM scheduler
    # pndm = PNDMScheduler.from_pretrained("sd-legacy/stable-diffusion-v1-5", subfolder="scheduler")
    # pndm.save_pretrained("pndm")
    pndm = PNDMScheduler.from_pretrained("pndm")
    fig, ax = plt.subplots(1, 4, figsize=(16, 4), num='PNDM')
    for ii, t in enumerate([0,10,100,999]):
        t = torch.tensor([ii for _ in range(8)])
        add_noise = pndm.add_noise(image, noise, t)
        ax[ii].imshow(add_noise[0, 0].cpu().numpy())
