...
def train(par_dir, conf, trial, experiment_name):
   # Read the config file #
    ...
    if diffusion_model_config['initialization'] == 'lora':
        logging.info('Using LoRA finituning on pretraing model')
    else:
        raise ValueError('Only LoRA initialization is supported')
    ...
    mixed_precision = precision_dict[train_config['mixed_precision']]

    text_encoder.to(accelerator.device, dtype=mixed_precision)
    clip_vision_model.to(accelerator.device, dtype=mixed_precision)
    vae.to(accelerator.device, dtype=mixed_precision)
    
    model.to(accelerator.device, dtype=mixed_precision)
    model.add_adapter(unet_lora_config)
    if train_config['mixed_precision'] == "fp16": # only upcast trainable parameters (LoRA) into fp32
    ...
    optimizer = torch.optim.AdamW(lora_layers, lr=train_config['ldm_lr'])   # optimizer = Adam(model.parameters(), lr=train_config['ldm_lr'])
